...
**Trip selection is redesigned** — see the README's "Trip selection" section and `Etappe Trips.dc.html`. Each trip becomes a horizontal card: 236×152 hero photo with a status pill, a three-tile highlight strip, a facts column (title, dates, mono `days · stops · km · cost band`, contributor pills), and a right rail with last-edited and the action. `New trip` and `Import a trip` move into the header — the always-open title/date form goes away and opens on demand. Photos resolve starred wishlist photo → unstarred → `kind`-tinted plate; never stock, never an empty frame.
