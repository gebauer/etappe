...
| `amenities` | json | `{key: "yes" \| "book"}` map over a closed set (`src/lib/amenities.ts`); accommodation only — what the place provides, so a companion knows what to pack. Absent key = bring your own. `"book"` (breakfast only) = available but must be reserved ahead |
